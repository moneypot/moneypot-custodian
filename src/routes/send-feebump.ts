...
export default async function sendFeeBump(feebump: hi.FeeBump) {
  ...
  if (isNew) {
    ...
    // wrap around transaction for continuity (not really required), also think it's a bit cleaner 

    // we can return ackdclaimable immediately, no await
    withTransaction(async (dbClient) => {
      (async function () {
        ...
        const getStatuses = await dbClient.query(
       ...
       `,
        ...
        for (const { status } of getStatuses.rows) {
          const getClaimables = await dbClient.query(
