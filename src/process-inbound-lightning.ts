...
export default async function processInboundLightning() {
  ...
  const { rows } = await db.pool.query(`SELECT * FROM lightning_invoices ORDER BY settle_index DESC NULLS LAST LIMIT 1;`);
  if (rows.length === 1) {
  ...
  lightning.subscribeSettledInvoices(lastSettleIndex, async invoice => {
    ...
    await db.pool.query(`UPDATE lightning_invoices SET
      r_preimage = $1,
      settle_index = $2,
      settle_amount = $3
      WHERE r_hash = $4`, [
