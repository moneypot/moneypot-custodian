...
export async function bySourceHash(sourceHash: string): Promise<PrunedTransfer | undefined> {

  const res = await pool.query(`
    SELECT output_hash, acknowledgement FROM transfers WHERE source_hash = $1
  `, [sourceHash]);
